    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      {adding && (
        <Card className="p-4 sm:p-6">
          <form
            onSubmit={handleAdd}
            className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 items-end"
          ...
          >
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <Input id="curl" name="curl" placeholder="https://..." aria-describedby="url-help" />
            </div>
            <Button
              type="submit"
              size="sm"
              className="sm:col-span-1"
              aria-label="Guardar nueva certificación"
            >
              Guardar
            </Button>
            <p id="url-help" className="sr-only">
              Ingresa la URL de credencial opcional para verificación
            </p>
          </form>
        </Card>
      )}
        ...
        <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
          {certs.map((c) => (
            <Card key={c.id} className="p-4 sm:p-5 hover:shadow-md transition-shadow">
              ...
              {c.issue_date && (
                <p className="text-xs text-muted-foreground">Emitida: {c.issue_date}</p>
              )}
              {c.credential_url && (
                <a
