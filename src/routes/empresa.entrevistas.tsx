...
const statusColors: Record<string, string> = {
  scheduled: "bg-primary/15 text-primary", confirmed: "bg-chart-2/15 text-chart-2",
  completed: "bg-muted text-muted-foreground", cancelled: "bg-destructive/15 text-destructive",
...
function Entrevistas() {
  ...
  useEffect(() => { listMyInterviews().then((r) => setInterviews(r.data)).catch(() => {}).finally(() => setLoading(false)); }, []);
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <PageHeader title="Entrevistas" description="Coordina y haz seguimiento de todas las entrevistas." />
      <Tabs defaultValue="proximas">
        <TabsList><TabsTrigger value="proximas">Próximas ({upcoming.length})</TabsTrigger><TabsTrigger value="pasadas">Historial ({past.length})</TabsTrigger></TabsList>
        <TabsContent value="proximas" className="mt-4 space-y-3">
          {upcoming.length === 0 ? <Card className="p-10 text-center text-muted-foreground"><p>No hay entrevistas próximas.</p></Card> :
            upcoming.map((i) => <InterviewCard key={i.id} iv={i} />)}
        </TabsContent>
        <TabsContent value="pasadas" className="mt-4 space-y-3">
          {past.length === 0 ? <Card className="p-10 text-center text-muted-foreground"><p>No hay historial.</p></Card> :
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground flex items-center gap-1"><Calendar className="h-3 w-3" />{iv.scheduled_at?.slice(0, 16) ?? "—"}</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          {iv.duration_minutes > 0 && <span className="text-xs text-muted-foreground flex items-center gap-1"><Clock className="h-3 w-3" />{iv.duration_minutes}m</span>}
          {iv.location_or_link && <span className="text-xs text-muted-foreground max-w-[150px] truncate">{iv.location_or_link}</span>}
