    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <Tabs defaultValue="catalogo">
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="catalogo" className="text-xs sm:text-sm">
            Catálogo ({evals.length})
          </TabsTrigger>
            ...
            <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
              {evals
                ...
                .map((ev) => (
                  <Card
                    key={ev.id}
                    className="p-4 sm:p-5 hover:shadow-md transition-shadow flex flex-col"
                  >
                    ...
                    <Button asChild size="sm" variant="outline" className="w-full">
                      ...
                      >
                        <ChevronRight className="h-3.5 w-3.5 mr-1" /> Tomar
                      </a>
                    </Button>
                  </Card>
                ))}
          ...
          {results.length === 0 ? (
            ...
            <div className="space-y-2 sm:space-y-3">
              {results.map((r) => (
                <Card
                  key={r.result.id}
                  className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 hover:shadow-sm transition-shadow"
                >
                  <div className="min-w-0">
                    <p className="font-medium text-sm sm:text-base">
                      {r.evaluation_title ?? r.result.evaluation_id}
                    </p>
                  ...
                  <Badge
                    className={`text-xs sm:text-sm ${r.result.passed ? "bg-primary text-primary-foreground" : "bg-destructive/15 text-destructive"} whitespace-nowrap`}
                  >
                    {r.result.passed
                      ? `Aprobado (${r.result.score})`
                      : `Reprobado (${r.result.score})`}
                  </Badge>
                </Card>
              ))}
            </div>
