    ...
    <Link to="/app/certificaciones/$id" params={{ id: cert.id }} className="block">
      <div className="relative" style={{ width: px, height: px }}>
        <svg viewBox="0 0 100 100" className="w-full h-full">
          ...
          <foreignObject x="20" y="30" width="60" height="40">
            <div className="h-full w-full grid place-items-center text-center">
              <div>
                <div className="text-[8px] font-mono uppercase tracking-widest font-bold" style={{ color: config.fillColor }}>
                  {cert.level}
                </div>
              </div>
            </div>
        ...
        {status !== "verified" && status !== "declared" && (
          <div className={`absolute top-1 right-1 h-2.5 w-2.5 rounded-full border-2 border-white ${status === "pending" ? "bg-amber-400 animate-pulse" : "bg-red-400"}`} />
        )}
        {status === "verified" && (
      ...
      {size !== "sm" && (
        <div className="mt-2 text-center space-y-1">
          ...
          <Badge variant="outline" className={`text-[10px] font-medium mx-auto ${config.statusBg} ${config.statusText} border-0`}>
            {config.statusLabel}
          </Badge>
        </div>
      )}
