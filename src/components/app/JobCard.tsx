...
export function JobCard({ job, match }: { job: Job; match?: { score: number; reason: string } }) {
  ...
  return (
    <Link to="/app/vacantes/$id" params={{ id: job.id }} aria-label={`${job.title} - ${matchConfig.label}`}>
      <Card className={`p-4 sm:p-5 md:p-6 hover:border-primary/40 transition-all h-full hover:shadow-md active:shadow-sm border-2 ${matchConfig.bg} cursor-pointer`} role="article">
        <div className="flex flex-col sm:flex-row sm:items-start gap-3 sm:gap-4">
          <div className="h-10 w-10 sm:h-11 sm:w-11 md:h-12 md:w-12 rounded-md bg-muted grid place-items-center font-mono font-bold text-sm flex-none flex-shrink-0 min-h-10 min-w-10">
            {job.title.slice(0, 2).toUpperCase()}
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex flex-col gap-2">
              <div className="min-w-0 flex-1">
                <p className="font-semibold text-sm sm:text-base leading-tight line-clamp-2">{job.title}</p>
                {match?.reason && (
              ...
              {match && (
                <div className="flex items-center justify-between sm:justify-start sm:flex-col sm:items-end gap-2">
                  <div className="flex items-center gap-1.5">
                    <div className={`px-2 sm:px-2.5 py-1 sm:py-1.5 rounded font-bold text-sm whitespace-nowrap ${matchConfig.badge}`}>
                      {matchScore}%
                    </div>
            ...
            <div className="mt-2 sm:mt-3 flex flex-wrap items-center gap-x-2 sm:gap-x-3 gap-y-1 text-xs text-muted-foreground">
              {job.location && (
                <span className="flex items-center gap-1 min-w-0"><MapPin className="h-3 w-3 flex-shrink-0" /> <span className="truncate">{job.location}</span></span>
              )}
              {job.is_remote && <span className="flex-shrink-0">Remoto</span>}
              {job.job_type && <span className="flex-shrink-0">·</span>}{job.job_type && <span className="truncate">{job.job_type}</span>}
            </div>
            {matchScore >= 80 && (
              <div className="mt-2 sm:mt-3 inline-flex items-center gap-1 text-xs font-semibold text-primary">
                Ver por qué <ChevronRight className="h-3 w-3 flex-shrink-0" />
              </div>
            )}
          </div>
        </div>
