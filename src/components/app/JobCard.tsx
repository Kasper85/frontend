...
export function JobCard({ job, match }: { job: Job; match?: { score: number; reason: string } }) {
  ...
  return (
    ...
    >
      <Card
        className={`p-4 sm:p-5 md:p-6 hover:border-primary/40 transition-all h-full hover:shadow-md active:shadow-sm border-2 ${matchConfig.bg} cursor-pointer`}
        role="article"
      >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-col gap-2">
              <div className="min-w-0 flex-1">
                ...
                {match?.reason && (
                  <p className="mt-1 text-xs text-muted-foreground line-clamp-2">{match.reason}</p>
                )}
              </div>
              {match && (
